...
export const useTokenWrapperStore = defineStore('tokenWrapper', () => {
  ...
  function calculateTokenStats(
    ...
    // Filter input token transfers (POLIS/ATLAS) for cost basis calculation
    const filteredInputTransfers = inputTokenTransfers.filter(t => {
      if (endDateTimestamp && t.timestamp) {
        const txDate = new Date(t.timestamp).getTime()
        return txDate <= endDateTimestamp
      }
    ...
    const walletCostBasisMap = new Map<string, number>() // Track POLIS/ATLAS paid

    // Both tokens: mints + transfersIn - burns - transfersOut
    
    // Process mints (both tokens)
    filteredMints.forEach(mint => {
      const owner = mint.solanaAccountByAccount?.owner
      if (owner) {
        const amount = parseFloat(mint.amountRaw || '0') / DECIMAL_DIVISOR
        const existing = walletHoldingsMap.get(owner) || 0
        walletHoldingsMap.set(owner, existing + amount)
      }
    })

    // Process transfers (both tokens)
    filteredTransfers.forEach(transfer => {
      const amount = parseFloat(transfer.amountRaw || '0') / DECIMAL_DIVISOR
      
      // Add to recipient (transfer in) - use owner if available, otherwise use account
      const toOwner = transfer.solanaAccountByToAccount?.owner || transfer.toAccount
      if (toOwner) {
      ...
      // Subtract from sender (transfer out) - use owner if available, otherwise use account
    ...
    // Process burns (both tokens - subtract from owner)
    ...
    // Calculate cost basis: track POLIS/ATLAS paid during minting
    // Match input token transfers with mints by instruction signature/timestamp
    const mintsBySignature = new Map<string, TokenMint>()
    const mintsByKey = new Map<string, TokenMint[]>()
    ...
    // Process input token transfers that match mints (wrap transactions)
    // Only process transfers that actually match a mint to avoid unnecessary work
    filteredInputTransfers.forEach(transfer => {
      let matchingMint: TokenMint | undefined
      ...
      if (matchingMint) {
        // This is a wrap transaction - the transfer amount is what was paid
        const inputAmount = parseFloat(transfer.amountRaw || '0') / DECIMAL_DIVISOR
        const mintOwner = matchingMint.solanaAccountByAccount?.owner
        if (mintOwner) {
          // Add to cost basis for the wallet that received the mint
          const existingCost = walletCostBasisMap.get(mintOwner) || 0
          walletCostBasisMap.set(mintOwner, existingCost + inputAmount)
        }
      }
    })
    ...
    const walletStats: WalletStats[] = Array.from(walletHoldingsMap.entries())
      .map(([wallet, holdings]) => {
        ...
        // Calculate current value using wrap ratio
        let currentValue = 0
        if (wrapRatio && wrapRatio.ratio > 0) {
          // Current value = holdings * wrap ratio (how much input token the holdings are worth)
          currentValue = holdings * wrapRatio.ratio
        }
